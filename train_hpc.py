...
import test_hpc  # import test.py to get mAP after each epoch
from models import *
from utils.datasets_hpc import *
...
def train(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if not (opt.notest or (opt.nosave and epoch < 10)) or epoch == epochs - 1:
            with torch.no_grad():
                results, maps = test_hpc.test(cfg, data_cfg, batch_size=batch_size, img_size=img_size_test, model=model,
                                          conf_thres=0.1)
